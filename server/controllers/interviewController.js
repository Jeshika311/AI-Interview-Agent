...
export const generateQuestion = async (req, res) => {
  try {
    ...
    if(user.credits < 50) {
      return res.status(403).json({ message: "Insufficient credits. Please recharge your account." });
    }
    ...
    const messages = [
      {
        ...
        content: `
          ...
          Make questions based on the candidate's role, experience, interviewMode, projects, skills, and resume content.
          `
      },
    ...
    return res.status(500).json({ message: "An error occurred while generating the question." });
  }
}
...
export const finishInterview = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "An error occurred while finishing the interview." });
  }
}
...
export const getInterviewReport = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "An error occurred while fetching the interview report." });
  }
}